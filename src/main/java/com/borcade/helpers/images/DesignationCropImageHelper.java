 ...
 * @since JDK1.8
 */
public class DesignationCropImageHelper extends BaseImageWatermarkHelper {
    private static final long serialVersionUID = 2127582300400169465L;

    public DesignationCropImageHelper(BaseImageWatermark imageWatermark) {
    ...
    public void applyWatermarkOnPicture(BufferedImage originalImage, Graphics2D originalGraphics) throws Exception {
        checkParam();
        CheckParams.checkFontWaterParams(imageWatermark, "newWatermarkWidth,newWatermarkHeight,transparency");
        // 设置水印旋转度
    ...
    private boolean isTransparentImageWatermark(Image watermarkImage) {
    ...
    private BufferedImage dealTransparentImageWatermark(BufferedImage originalImage, Graphics2D originalImageGraphics, Image watermarkImage) {
        BufferedImage afterDealWatermark = new BufferedImage(imageWatermark.getNewWatermarkWidth(),
                imageWatermark.getNewWatermarkHeight(), Transparency.TRANSLUCENT);
        applyWatermarkInImage(originalImage, originalImageGraphics, watermarkImage, afterDealWatermark);
        return afterDealWatermark;
    }


    private BufferedImage dealOpaqueImageWatermark(BufferedImage originalImage, Graphics2D originalImageGraphics, Image watermarkImage) {
        //处理之后的水印
        BufferedImage afterDealWatermark = new BufferedImage(imageWatermark.getNewWatermarkWidth(),
                imageWatermark.getNewWatermarkHeight(), BufferedImage.TYPE_INT_RGB);
        applyWatermarkInImage(originalImage, originalImageGraphics, watermarkImage, afterDealWatermark);
        return afterDealWatermark;
    }
    ...
    private void applyWatermarkInImage(BufferedImage originalImage, Graphics2D originalImageGraphics, Image watermarkImage, BufferedImage afterDealWatermark) {
        ...
        //绘制等比裁剪后的水印
        waterGraphics.drawImage(watermarkImage.getScaledInstance(imageWatermark.getNewWatermarkWidth(),
                imageWatermark.getNewWatermarkHeight(), Image.SCALE_SMOOTH), 0, 0, null);
        //释放资源
        ...
        //绘制水印
        originalImageGraphics.drawImage(watermarkIconImage, originalImage.getWidth() * imageWatermark.getWidth() / 100,
